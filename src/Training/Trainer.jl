...
function train_epoch!(model::CassandraModel, opt_state, dataset_path::AbstractString;
    ...
    for (tensors, values, policy_idxs, legal_mask, sample_weights) in batch_iterator(reader, batch_size; shuffle=false)
        targets = Flux.onehotbatch(policy_idxs, 1:N_MOVES)
        tensors_d    = tensors         |> dev
        values_d     = values          |> dev
        targets_d    = targets         |> dev
        legal_mask_d = legal_mask      |> dev
        weights_d    = sample_weights  |> dev
        w_sum        = max(sum(weights_d), 1f-6)

        (total, (lv, lp)), grads = Flux.withgradient(model) do m
            value_preds, policy_logits = m(tensors_d)
            # Weighted MSE for value head.
            per_lv = (vec(value_preds) .- values_d).^2
            lv_ = sum(weights_d .* per_lv) / w_sum
            ...
            # Weighted cross-entropy for policy head.
            per_lp = vec(-sum(targets_d .* Flux.logsoftmax(masked_logits; dims=1); dims=1))
            lp_ = sum(weights_d .* per_lp) / w_sum
...
"""
    train_epoch!(model, opt_state, dataset_paths, mixing_weights; kwargs...) -> TrainStats

Multi-dataset variant. Randomly mixes batches from multiple datasets each epoch.
`mixing_weights` are probabilities (need not sum to 1; they are normalised internally).
The total number of batches per epoch is determined by the weighted total record count.
"""
function train_epoch!(model::CassandraModel, opt_state,
                      ...
                      epoch::Int=0,
                      device=Flux.cpu_device())
    length(dataset_paths) == length(mixing_weights) ||
        error("dataset_paths and mixing_weights must have the same length")

    readers = [DatasetReader(p) for p in dataset_paths]
    # Probability of drawing a batch from each dataset, weighted by mixing_weights.
    # (Ignores dataset size — mixing_weights control the ratio directly.)
    w_total = sum(mixing_weights)
    probs   = mixing_weights ./ w_total
    cum_probs = cumsum(probs)

    # Total batches: weighted-average dataset size / batch_size.
    avg_records = sum(probs[i] * readers[i].n_records for i in eachindex(readers))
    n_batches_total = max(1, round(Int, avg_records / batch_size))
    ...
    for _ in 1:n_batches_total
        # Pick dataset by probability.
        u = rand()
        ri = findfirst(c -> u <= c, cum_probs)
        ri === nothing && (ri = length(readers))
        batch = random_batch(readers[ri], batch_size)

        tensors, values, policy_idxs, legal_mask, sample_weights = batch
        targets = Flux.onehotbatch(policy_idxs, 1:N_MOVES)
        tensors_d    = tensors        |> dev
        values_d     = values         |> dev
        targets_d    = targets        |> dev
    ...
    n_total = sum(r.n_records for r in readers)
    stats = TrainStats(total_lv/nb, total_lp/nb,
                       (value_weight*total_lv + policy_weight*total_lp)/nb,
                       nb, time() - t0)
